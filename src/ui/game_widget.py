...
class GameWidget(FloatLayout):
  def __init__(self, **kwargs):
    ...
    tricks_top = CardSlotsLayout(rows=2, cols=8, debug=True, align_top=True)
    tricks_top.size_hint = 1, 1
    tricks_bot = CardSlotsLayout(rows=2, cols=8, debug=True, align_top=True)
    tricks_bot.size_hint = 1, 1
    self.ids.computer_tricks.add_widget(tricks_top)
    self.ids.player_tricks.add_widget(tricks_bot)

    cards_bot = CardSlotsLayout(rows=1, cols=5, debug=True, align_top=True)
    tricks_bot.size_hint = 1, 1
    ...
    cards_top = CardSlotsLayout(aspect_ratio=24 / 37 / 0.1 * 0.35, rows=1,
                                cols=5, debug=True, align_top=True)
